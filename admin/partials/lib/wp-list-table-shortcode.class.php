 ...
 */
class Custom_List_Table_Shortcode extends WP_List_Table {
	...
	function get_sortable_columns() {
		$sortable_columns = array(
			...
			'template_shortcode' => array( 'template_shortcode', true ),
		);
	...
	protected function itechpublic_get_filter_count( $data = '' ) {
		...
		if ( $data == 1 ) {
			...
			$results = $wpdb->get_var( $wpdb->prepare( "SELECT count(*) FROM $table_name WHERE template_shortcode = %d", 1 ) );
		} else {
